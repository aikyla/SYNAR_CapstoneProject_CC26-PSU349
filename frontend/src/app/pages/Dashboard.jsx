...
import { fetchWeather, predictSkinType, reverseGeocode, saveHistory, searchLocations, predictManualCorrection, predictBestTime } from '../services/synarService';
...
export default function Dashboard() {
  ...
  const handleCheckUV = async () => {
    ...
    try {
      ...
      // 1. Ambil koreksi waktu aman menggunakan AI Regresi (Port 8001) untuk mode manual
      let safeTime = mlSafeTime;
      if (!safeTime) {
        try {
          const correctionRes = await predictManualCorrection(skinType, weather);
          if (correctionRes && correctionRes.safe_time) {
            safeTime = correctionRes.safe_time;
            setMlSafeTime(safeTime);
          }
        } catch (err) {
          console.error("Gagal memanggil AI Koreksi, menggunakan fallback:", err.message);
          if (normalizedUv > 0) {
            const MED_LOWER = { 1: 200, 2: 250, 3: 300, 4: 450, 5: 600, 6: 1000 };
            let dur = MED_LOWER[skinType] / (normalizedUv * 1.5);
            dur = Math.min(dur, 120.0);
            dur = Math.round(dur * 10) / 10;
            safeTime = dur >= 120 ? "> 120 mins" : (dur < 10 ? "< 10 mins" : `${dur} mins`);
          } else {
            safeTime = "> 120 mins";
          }
          setMlSafeTime(safeTime);
      ...
      // 2. Ambil Rekomendasi Best Time dari AI Regresi (Port 8001)
      try {
        const bestTimeRes = await predictBestTime(
          new Date().getMonth() + 1,
          coordsSnapshot.latitude,
          coordsSnapshot.longitude
        );
        if (bestTimeRes) {
          setBestTimeData(bestTimeRes);
        } else {
          setBestTimeData(null);
        }
      } catch (err) {
        console.error("Gagal mendapatkan rekomendasi Best Time:", err.message);
        setBestTimeData(null);
      }
  ...
  const simulateDetection = async () => {
    ...
    try {
      ...
      let displayTime = prediction.safe_time;
      if (!displayTime && prediction.duration_minutes !== undefined && prediction.duration_minutes !== null) {
        displayTime = `${prediction.duration_minutes} mins`;
      }
      setDetectedType(predictedType);
      setSkinType(predictedType);
      setMlSafeTime(displayTime || null);
      setMlRiskLevel(prediction.risk_level || null);
      setCameraState('detected');
    } catch (error) {
      setCameraError(error instanceof Error ? error.message : 'Unable to detect skin type.');
      setCameraState('active');
    }
  ...
  // Helper fungsi untuk menghitung waktu aman secara dinamis di render
  const getDisplayedSafeTime = () => {
    if (isNightTime) return 'UV exposure is minimal now';
    if (mlSafeTime) return mlSafeTime;
    if (uvData && uvData.index !== undefined) {
      const uvi = Math.round(uvData.index);
      if (uvi > 0) {
        const MED_LOWER = { 1: 200, 2: 250, 3: 300, 4: 450, 5: 600, 6: 1000 };
        let dur = MED_LOWER[skinType] / (uvi * 1.5);
        dur = Math.min(dur, 120.0);
        dur = Math.round(dur * 10) / 10;
        
        if (dur >= 120) return "> 120 mins";
        if (dur < 10) return "< 10 mins";
        return `${dur} mins`;
      }
      return "> 120 mins";
    }
              ...
              <AnimatePresence mode="wait">
                {detectMode === 'manual' ? (
                  <motion.div key="manual-view" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }} className="flex justify-between items-center gap-2 md:gap-4 bg-slate-50 p-3 rounded-3xl border border-slate-100">
                    {skinTypes.map((type) => (
                      <button
                        key={type.id} onClick={() => { setSkinType(type.id); setMlSafeTime(null); setMlRiskLevel(null); }}
                    ...
                    {cameraState === 'active' && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-sm aspect-[4/3] bg-slate-900 rounded-2xl relative overflow-hidden shadow-xl">
                        ...
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3">
                          {cameraError && <p className="text-sm text-red-200 bg-red-500/20 px-3 py-2 rounded-full border border-red-400/40">{cameraError}</p>}
          ...
          {loading ? (
            ...
            <motion.div key="results" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 200, damping: 20 }} className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-[32px] p-8 shadow-xl border border-white flex flex-col">
                ...
                {/* AI Best Outdoor Hours (Rekomendasi Waktu Keluar Teraman) */}
                {bestTimeData && (
                  <motion.div 
                    initial={{ opacity: 0, y: 10 }} 
                    animate={{ opacity: 1, y: 0 }}
                    className="mt-6 p-5 bg-gradient-to-r from-orange-50 to-amber-50 rounded-3xl border border-orange-100/50 flex flex-col space-y-4 shadow-sm"
                  >
                    <h4 className="font-black text-slate-800 flex items-center gap-2 text-sm uppercase tracking-wider">
                      <Sparkles className="w-5 h-5 text-orange-500" /> AI Best Outdoor Hours
                    </h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-white/70 backdrop-blur-md rounded-2xl p-4 border border-emerald-100 shadow-sm text-left">
                        <p className="text-xs font-black text-emerald-600 uppercase tracking-widest mb-2.5">🟢 Best Hours</p>
                        <div className="flex flex-wrap gap-1.5">
                          {bestTimeData.best_hours?.map(h => (
                            <span key={h} className="px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-black rounded-lg border border-emerald-100">{h}</span>
                          ))}
                        </div>
                      </div>
                      <div className="bg-white/70 backdrop-blur-md rounded-2xl p-4 border border-red-100 shadow-sm text-left">
                        <p className="text-xs font-black text-red-500 uppercase tracking-widest mb-2.5">🔴 Avoid Hours</p>
                        <div className="flex flex-wrap gap-1.5">
                          {bestTimeData.worst_hours?.map(h => (
                            <span key={h} className="px-2.5 py-1 bg-red-50 text-red-700 text-xs font-black rounded-lg border border-red-100">{h}</span>
                          ))}
                        </div>
                      </div>
                    </div>
