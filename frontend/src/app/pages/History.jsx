            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center bg-gradient-to-br ${uv.gradient} text-white shadow-md shadow-orange-500/10 shrink-0`}>
                    <span className="text-xs font-bold uppercase opacity-85 leading-none">UV</span>
                ...
                <div className="flex flex-wrap items-center gap-2.5 mt-2 sm:mt-0">
                  {item.skin_type && (
                    <div className="px-3 py-1.5 rounded-xl font-bold text-xs bg-orange-50 text-orange-700 border border-orange-100 shadow-sm shrink-0">
                      Skin Type {item.skin_type}
                    </div>
                  )}
                  {item.recommended_duration && (
                    <div className="px-3 py-1.5 rounded-xl font-black text-xs bg-emerald-50 text-emerald-700 border border-emerald-100 shadow-sm flex items-center gap-1 shrink-0">
                      ⏱️ {item.recommended_duration}
                    </div>
                  )}
                  <div className={`px-4 py-1.5 rounded-full font-bold text-sm tracking-wider uppercase bg-white ${uv.color} border border-current/20 shadow-sm`}>
                    {item.risk_level || uv.risk}
                  </div>
                  <button onClick={() => handleDelete(item.historyId)} disabled={deletingId === item.historyId} className="w-10 h-10 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 disabled:opacity-50 flex items-center justify-center transition-colors border border-red-100" aria-label="Delete history">
